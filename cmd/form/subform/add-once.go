...
func HandleAddOnceCommand(
	...
	if(mention == "me"){
		...
		err = subfunc.WriteFile("form.txt", fmt.Sprintf("%s, %s, %d, %d, %d, %d, %s", channel.ID, channelName, year, month, day, hour, mention))
		if err != nil {
		...
		err = subfunc.WriteFile("form.txt", fmt.Sprintf("%s, %s, %d, %d, %d, %d, %s", channelID, channelName, year, month, day, hour, mention))
		if err != nil {
			log.Printf("failed to write data to file: %v", err)
			return
		}
